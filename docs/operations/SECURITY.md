...
| Access control (authZ) | 🟡 | Postgres RLS by org/role; parity not fully verified. |
...
- **Tenant isolation**: RLS keyed on `current_user_org_id()` / role.
...
| ----- | ------ | ---------- | ------------- |
| PHI in Postgres | Cross-tenant read | RLS by org/role | RLS parity not fully audited → **medium** |
...
4. **RLS parity unverified** for every table (Phase 1).
...
- Verified RLS; systematic audit logging; CSP; TLS/DNS.
