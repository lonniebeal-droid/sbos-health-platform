...
3. **Database isolation → local Supabase RLS verifier.** Requires `supabase start`
   ...
   own applied-migration ledger — see [DATABASE](DATABASE.md)); runs 18
   ...
   behavior against the seeded DB. Every migration's DDL was validated
   syntax/reference-clean via BEGIN/ROLLBACK against the live project, but
   `supabase start` itself could not be run in this environment — Docker
   registry pulls are blocked by network policy here (confirmed via the
   proxy status endpoint: 403 policy denials on the image registries),
   Supabase branching needs a paid plan, and a new free project is blocked
   by this account's 2-project limit. Treat `verify-rls.sh`'s assertions as
   reviewed-correct, not empirically confirmed, until run somewhere unblocked.
...
- `scripts/verify-rls.sh` has not actually been executed end-to-end — needs a
  run against `supabase start` somewhere Docker registry pulls aren't
  network-policy-blocked, to confirm the reviewed-but-unverified checks
  actually pass.
