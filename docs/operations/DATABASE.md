...
The first 18 filenames and SQL bodies below were recovered from the live
...
- **Known, disclosed limitations** (not silently assumed solved):
  ...
  - The exact 18 historical migrations plus the forward grants migration
    were replayed locally with `supabase db reset`; `seed.sql` completed and
    `npm run verify:rls` passed all 18 checks. The forward grants migration
    is staged in source only, so the hosted project's broad effective table
    grants remain a separate deployment review item.
